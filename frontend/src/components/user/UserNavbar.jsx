...
const UserNavbar = () => {
  ...
  const hideSearchPages = ["/login", "/sign-up", "/contact", "/about"];
      ...
      <div className="bg-white text-gray-900 w-full fixed top-[28px] left-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto flex justify-between items-center px-6 mb-4 sm:mt-8 md:mt-10">
          ...
          <div className="hidden md:flex space-x-6 items-center">
            ...
            {!user ? (
              <Link to="/login" className="text-gray-700 hover:text-gray-900 font-medium transition">Login</Link>
            ) : (
              <>
                <Link to="/user-Dashboard" className="text-gray-700 hover:text-gray-900 font-medium transition">Dashboard</Link>
                <button onClick={handleLogout} className="text-red-600 hover:text-red-800 font-medium transition">Logout</button>
              </>
            )}
          </div>

          {/* Search + Wishlist + Cart (desktop) */}
          {showSearch && (
            <div className="hidden md:flex items-center space-x-4">
              ...
              {/* Wishlist */}
              <Link to="/wish-list" className="text-gray-700 hover:text-gray-900 transition">
              ...
              {/* Cart */}
              <Link to="/cart-page" className="text-gray-700 hover:text-gray-900 transition">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
